...
include::api-tests/bruno/startrek-ships/prometheus_reread_config.bru[lines=8..8]    
    

== Developement
In general i'll stick to building the API in a TDD way. Tests are written in https://docs.pytest.org/en/stable/[pytest].
Implemenrtation of pytest-bdd enables us to write business driven tests. Please refer to `get_ship.feature` and it implementation `get_ship.py` for details.
...
| http://localhost:9093/#/alerts[AlertManager] | NOK
| http://localhost:1080[Fake SMTP] Apps may send SMTP Mails to Pot 1025 | NOK
|=== 
...
- Add https://github.com/haravich/fake-smtp-server[fake smtp] server to enanble local test of alertmanager and grafana
